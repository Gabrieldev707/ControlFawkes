...
function _makeCircleSprite() {
  ...
  gradient.addColorStop(0.5, `rgba(255,255,255,${ORB_VISUAL_TUNING.spriteMidHaloOpacity})`);
...
export class FawkesOrb {
  ...
  private targetBright: number = ORB_VISUAL_TUNING.initialPointOpacity;
  ...
  private currentBright: number = ORB_VISUAL_TUNING.initialPointOpacity;
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this.mat = new THREE.PointsMaterial({
      ...
      alphaMap:        sprite,
      transparent:     true,
      opacity:         ORB_VISUAL_TUNING.initialPointOpacity,
    ...
    this.electronMat = new THREE.PointsMaterial({
      ...
      alphaMap:        sprite,
      transparent:     true,
      opacity:         ORB_VISUAL_TUNING.electronOpacity,
      sizeAttenuation: true,
      blending:        THREE.AdditiveBlending,
      depthWrite:      false,
      alphaTest:       0.01,
  ...
  private _update(): void {
    ...
    if (this.lineAmount > 0.01) {
      ...
      lp.needsUpdate = true;
      lc.needsUpdate = true;
      this.lineMat.opacity = this.lineAmount * ORB_VISUAL_TUNING.lineOpacityMultiplier;
