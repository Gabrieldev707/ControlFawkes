...
def test_spotify_search_uses_official_encoded_uri_without_choosing_a_result():
    uri_opener = Mock()
    browser = Mock(spec=BrowserLauncher)
    launcher = SpotifyLauncher(browser_launcher=browser, uri_opener=uri_opener)

    result = launcher.search("Águas de Março")

    assert result.executed is True
    assert result.strategy == "SPOTIFY_APP"
    uri_opener.assert_called_once_with("spotify:search:%C3%81guas%20de%20Mar%C3%A7o")
    browser.open.assert_not_called()


def test_spotify_search_falls_back_to_encoded_web_search_in_chrome():
    uri_opener = Mock(side_effect=OSError("protocol unavailable"))
    browser = Mock(spec=BrowserLauncher)
    browser.open.return_value = BrowserLaunchResult(executed=True, strategy="CHROME")
    launcher = SpotifyLauncher(browser_launcher=browser, uri_opener=uri_opener)

    result = launcher.search("Billie Jean Michael Jackson")

    assert result.executed is True
    assert result.strategy == "SPOTIFY_WEB_CHROME"
    browser.open.assert_called_once_with(
        "https://open.spotify.com/search/Billie%20Jean%20Michael%20Jackson"
    )
